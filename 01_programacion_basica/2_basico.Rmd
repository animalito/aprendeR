---
title: "Vectorización, la familia apply y el paradigma S-A-C"
output:
  pdf_document:
    fig_caption: yes
...
etc. \textcite{probdistributions} resumen otros paquetes en donde se encuentran
...
después combinas los resultados obtenidos \parencite{wickham2011split}.

Esta estrategia se utiliza en diversas etapas del análisis de datos, por ejemplo \parencite{wickham2011split}:
...
el argumento `by` disponible en algunos procedimientos de `SAS` \parencite{wickham2011split}.
...
`plyr` es la primera versión de SAC de Wickham \parencite{wickham2011split}. 
Posteriormente, mejoró
muchas de las funciones en `dplyr` \parencite{dplyr} sobretodo entorno a velocidad y facilidad de
uso. `plyr` no termina de ser relevante pues varias de sus funciones aun no están
en `dplyr` pero está por ser sustituido.

`data.table` \parencite{datatable}, es una implementación con una tradición muy diferente y tiene
