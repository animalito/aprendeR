...
\hypersetup{unicode=true,
            pdftitle={Vectorización, la familia apply y el paradigma S-A-C},
            pdfborder={0 0 0},
            breaklinks=true}
...
\setlength{\droptitle}{-2em}
  \title{Vectorización, la familia apply y el paradigma S-A-C}
...
En este capítulo se profundiza en la forma en la que es posible operar
los distintos objetos examinados en el capítulo anterior. En particular,
se revisa con detalle las diferentes maneras con las que se pueden
extraer \emph{subconjuntos} de las estructuras de datos.

Para \emph{vectores}, se detallan los seis tipos de extracciones y,
posteriormente, se describe cómo esos seis métodos aplican en
\emph{matrices} y \emph{dataframes}.

Se describe la asignación de valores a los subconjuntos extraídos, así
como los operadores lógicos disponibles en \texttt{R} y sus
particularidades.

Se introducen también distintas aplicaciones del material revisados de
forma que sea posible realizar operaciones que típicamente se hacen en
Excel, como buscarv o buscarh. Se revisa cómo expandir una base con
pesos, cómo seleccionar muestras aleatorias y cómo generar datos a
partir de realizaciones de distribuciones específicas implementadas en
\texttt{R}.

Por último, se revisa la estrategia separa, aplica y combina (SAC), así
como su implementación en \texttt{R} básico en la familia \texttt{apply}
Esta estrategia permitirá que fácilmente se realicen operaciones
complejas sobre grupos, especificando explícitamente la estructura de
datos de entrada y de salida que se está buscando.
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{str}\NormalTok{(df}\OperatorTok{$}\NormalTok{x)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{impares <-}\StringTok{ }\DecValTok{1}\OperatorTok{:}\DecValTok{10} \OperatorTok{%%}\StringTok{ }\DecValTok{2} \OperatorTok{==}\StringTok{ }\DecValTok{1}
\NormalTok{mult.}\DecValTok{3}\NormalTok{ <-}\StringTok{ }\DecValTok{1}\OperatorTok{:}\DecValTok{10} \OperatorTok{%%}\StringTok{ }\DecValTok{3} \OperatorTok{==}\StringTok{ }\DecValTok{0}
 
\NormalTok{impares }\OperatorTok{&}\StringTok{ }\NormalTok{mult.}\DecValTok{3}
\NormalTok{impares }\OperatorTok{|}\StringTok{ }\NormalTok{mult.}\DecValTok{3}

\KeywordTok{xor}\NormalTok{(impares, mult.}\DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{aprueba <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}
  \DataTypeTok{calificacion =} \DecValTok{10}\OperatorTok{:}\DecValTok{1}\NormalTok{,}
  \DataTypeTok{descripcion =} \KeywordTok{c}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\StringTok{"excelente"}\NormalTok{, }\DecValTok{2}\NormalTok{), }\StringTok{"bueno"}\NormalTok{, }\KeywordTok{rep}\NormalTok{(}\StringTok{"aceptable"}\NormalTok{, }\DecValTok{2}\NormalTok{)}
\NormalTok{                  , }\KeywordTok{rep}\NormalTok{(}\StringTok{"no satisfactorio"}\NormalTok{, }\DecValTok{5}\NormalTok{)),}
...
\NormalTok{)}
\NormalTok{id <-}\StringTok{ }\KeywordTok{match}\NormalTok{(calificaciones, aprueba}\OperatorTok{$}\NormalTok{calificacion)}
...
\subsection{Which, intersect y union}\label{which-intersect-y-union}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pares <-}\StringTok{ }\DecValTok{1}\OperatorTok{:}\DecValTok{10} \OperatorTok{%%}\StringTok{ }\DecValTok{2} \OperatorTok{==}\StringTok{ }\DecValTok{0}
\NormalTok{m.}\DecValTok{5}\NormalTok{ <-}\StringTok{ }\DecValTok{1}\OperatorTok{:}\DecValTok{10} \OperatorTok{%%}\StringTok{ }\DecValTok{5} \OperatorTok{==}\StringTok{ }\DecValTok{0}

\KeywordTok{c}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{10}\NormalTok{)[}\KeywordTok{union}\NormalTok{(}\KeywordTok{which}\NormalTok{(pares), }\KeywordTok{which}\NormalTok{(m.}\DecValTok{5}\NormalTok{))]}
\KeywordTok{c}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{10}\NormalTok{)[}\KeywordTok{intersect}\NormalTok{(}\KeywordTok{which}\NormalTok{(pares), }\KeywordTok{which}\NormalTok{(m.}\DecValTok{5}\NormalTok{))]}
\KeywordTok{c}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{10}\NormalTok{)[}\KeywordTok{which}\NormalTok{(}\KeywordTok{xor}\NormalTok{(pares, m.}\DecValTok{5}\NormalTok{))]}
\end{Highlighting}
\end{Shaded}

\subsection{Generación de datos a partir de
distribuciones}\label{generacion-de-datos-a-partir-de-distribuciones}

\texttt{R} básico provee funciones de distribución de probabilidad (p),
funciones de densidad (d), funciones cuantiles (q) y generación de
números aleatorios (r) para varias distribuciones como la binomial,
chi-cuadrada, gamma, geométrica, etc. \textcite{probdistributions}
resumen otros paquetes en donde se encuentran implementadas otras
funcionalidades o distribuciones multivariadas y cópulas.

En las funciones de \texttt{R} básico, podemos construir un
\emph{dataframe} con realizaciones de distintas distintas
distribuciones, por ejemplo, a continuación generamos la variable \(x\)
como realizaciones de una normal, \(y\) con realizaciones de una
exponencial y \(z\) con realizaciones de una uniforme.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{df <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}
  \DataTypeTok{x =} \KeywordTok{rnorm}\NormalTok{(}\DecValTok{100}\NormalTok{)}
\NormalTok{  , }\DataTypeTok{y =} \KeywordTok{rexp}\NormalTok{(}\DecValTok{100}\NormalTok{)}
\NormalTok{  , }\DataTypeTok{z =} \KeywordTok{runif}\NormalTok{(}\DecValTok{100}\NormalTok{)}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\ControlFlowTok{for}\NormalTok{ (l }\ControlFlowTok{in} \KeywordTok{unique}\NormalTok{(df}\OperatorTok{$}\NormalTok{x) )\{}
  \KeywordTok{print}\NormalTok{(}\KeywordTok{paste0}\NormalTok{(}\StringTok{"Grupo con letra: "}\NormalTok{, l))}
  \KeywordTok{print}\NormalTok{(df[l }\OperatorTok{==}\StringTok{ }\NormalTok{df}\OperatorTok{$}\NormalTok{x, ])}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\ControlFlowTok{for}\NormalTok{ (l }\ControlFlowTok{in} \KeywordTok{unique}\NormalTok{(df}\OperatorTok{$}\NormalTok{x) )\{}
  ...
  \KeywordTok{print}\NormalTok{(}\KeywordTok{mean}\NormalTok{(df[l }\OperatorTok{==}\StringTok{ }\NormalTok{df}\OperatorTok{$}\NormalTok{x, ]}\OperatorTok{$}\NormalTok{y))}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\ControlFlowTok{for}\NormalTok{ (l }\ControlFlowTok{in} \KeywordTok{unique}\NormalTok{(df}\OperatorTok{$}\NormalTok{x) )\{}
\NormalTok{  medias[[l]] <-}\StringTok{ }\KeywordTok{mean}\NormalTok{(df[l }\OperatorTok{==}\StringTok{ }\NormalTok{df}\OperatorTok{$}\NormalTok{x, ]}\OperatorTok{$}\NormalTok{y)}
\NormalTok{\}}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{tapply}\NormalTok{(warpbreaks}\OperatorTok{$}\NormalTok{breaks, }
       \KeywordTok{list}\NormalTok{(}\DataTypeTok{wool =}\NormalTok{ warpbreaks}\OperatorTok{$}\NormalTok{wool, }\DataTypeTok{tension =}\NormalTok{ warpbreaks}\OperatorTok{$}\NormalTok{tension), }
\NormalTok{       mean)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{res <-}\StringTok{ }\KeywordTok{by}\NormalTok{(diamonds[, }\KeywordTok{c}\NormalTok{(}\StringTok{"carat"}\NormalTok{)], }
          \KeywordTok{list}\NormalTok{(}\DataTypeTok{cut =} \KeywordTok{as.factor}\NormalTok{(diamonds}\OperatorTok{$}\NormalTok{cut)}
\NormalTok{               , }\DataTypeTok{color =} \KeywordTok{as.factor}\NormalTok{(diamonds}\OperatorTok{$}\NormalTok{color))}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{replicate}\NormalTok{(}\DecValTok{5}\NormalTok{, }\KeywordTok{rnorm}\NormalTok{(}\DecValTok{5}\NormalTok{), }\DataTypeTok{simplify =}\NormalTok{ F)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{2_basico_files/figure-latex/unnamed-chunk-48-1.pdf}
...
\tightlist
\item
  Curso de \texttt{swirl} \textbf{R Programming}, módulos 10 a 15.
\item
  Curso \textbf{Intermediate R} de
  \href{https://www.datacamp.com/courses/intermediate-r}{Data Camp}.
\item
  Curso \textbf{Intermediate R - Practice} de
  \href{https://www.datacamp.com/courses/intermediate-r-practice}{Data
  Camp}.
\end{itemize}
