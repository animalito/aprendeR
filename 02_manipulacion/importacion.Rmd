...
| Stata                   | \hyperref[stata]{haven::read\_dta} | \hyperref[stata]{foreign::write.dta} |
...
| Heroku Postgres         | \hyperref[heroku-postgres]{sql2df} | \hyperref[heroku-postgres]{df2sql} |
...
```{r}
# haven - como tibble
misdatos <- read_sas("datos_sas/misdatos.sas7bdat", catalog_file = NULL, encoding = NULL)
```
...
sql2df <- function(sql.file, df.file = "") {
  ...
  if(!file.exists(df.file)) {
    ...
    # Leemos el query
    sql <- paste(readLines(sql.file,encoding="UTF-8"), sep=" ", collapse=" ")
...
La función `sql2df`\footnote{Función adaptada de notas de Adolfo de Únanue.} 
...
df2sql <- function(data.frame, df.schema.name, df.table.name, owner.to = NA) {
  ...
  tryCatch( {
    flog.info("Ejecutando la escritura de tabla %s en el esquema %s", df.table.name, df.schema.name)
    # Definimos el camino al esquema deseado
    if(df.schema.name != "public"){
      dbSendQuery(conn = con, statement = paste0("SET search_path = ", df.schema.name, ", public;"))
    }
    ...
    if(!is.na(owner.to)){
      ...
      dbSendQuery(con, paste0("alter table ", long.name, " owner to ", owner.to, ";"))
    }
