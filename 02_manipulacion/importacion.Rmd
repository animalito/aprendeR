...
Para aplicar las herramientas de \texttt{R} a nuestro trabajo, es necesario poder importar
nuestros datos a \texttt{R}. \texttt{R} tiene conectores ya implementados para casi cualquier 
...
| IBM SPSS                | \hyperref[ibm-spss]{haven::read\_sav} | \hyperref[ibm-spss]{haven::write\_sav} |
| Stata                   | \hyperref[stata]{haven::read\_dta} | \hyperref[stata]{haven::write\_dta} |
| SAS                     | \hyperref[sas]{haven::read\_sas} | \hyperref[sas]{haven::write\_sas} |
| Google spreadsheet      | \hyperref[google-spreadsheet]{googlesheets::gs\_read} | \hyperref[google-spreadsheet]{googlesheets::gs\_new} |
| Google bigquery         |  | \hyperref[google-bigquery]{bigrquery::query\_exec} |
| rdata                   | \hyperref[rdata]{base::load} | \hyperref[rdata]{base::save} |
...
\texttt{R} tiene conexión con muchos de los formatos en los que se encuentran 
...
comunmente encontrada en los ejemplos de la documentación de las funciones. 
...
medio del explorador de archivos, verás en esa carpeta el archivo `misdatos_locales.rds`.
...
ejemplo anterior leimos un *tibble* y el paquete `foreign` es más viejo (y no
...
Para usar el paquete `foreign` ejemplificaremos la creación de un directrio de
...
ahora una carpeta llamada `datos_sas`. Creamos ahí un archivo de texto plano
con los datos y un `script` de `SAS` para leerlos apropiadamente:
...
Nota como la tabla cuenta con aprox. 140 millones de registros y se obtiene el
detalle en segundos.

## rdata

Tambien es posible guardar objetos especificos del ambiente dentro de un formato 
